...
public class BoardManager : MonoBehaviour
{
    ...
    private Vector2 GetPieceBoardPos(Ball piece)
    {
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
    ...
    private bool IsMatchPiece(Ball piece)
    {
        // ピースの情報を取得
    ...
    private int GetSameKindPieceNum(BallGenarater kind, Vector2 piecePos, Vector2 searchDir)
    {
        var count = 0;
        while (true)
        {
